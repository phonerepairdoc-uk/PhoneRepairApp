...
def load_data():
    try:
        ...
        return pd.DataFrame(columns=["Device Model", "Issue", "Price"])
...
def save_repair_request(name, device, issue, cost):
    try:
        ...
        st.success("Repair request saved successfully!")
    except Exception as e:
        st.error(f"Error saving repair history: {e}")
...
def repair_inquiry_web_interface():
    ...
    image_path = device_images.get(device_type, "")
    if os.path.exists(image_path):
        st.image(image_path, caption=device_type, width=200)
    else:
        st.warning(f"Image not found for {device_type}")
